                    ...
                    <div className="space-y-8">
                       ...
                       {(currentSlide?.template === 'PREMIUM_GLASS' || currentSlide?.template === 'CINEMATIC_BG' || currentSlide?.template === 'FADE') && (
                         <div className="pt-6 border-t border-white/5 space-y-6">
                           ...
                           <div className="grid grid-cols-2 gap-6">
                             <TransformControl label={currentSlide?.template === 'PREMIUM_GLASS' ? ((imageConfig.boxOverlay || 'light') === 'dark' ? 'Potência do Dark Glass' : 'Potência do Light Glass') : 'Força do Overlay'} value={currentSlide?.options?.backgroundOverlayStrength ?? (currentSlide?.template === 'PREMIUM_GLASS' ? 0.42 : currentSlide?.template === 'CINEMATIC_BG' ? 0.32 : 0.55)} min={0} max={1} step={0.01} onChange={(v) => updateSlideProperty(['options', 'backgroundOverlayStrength'], v)} />
                             <TransformControl label="Blur de Leitura" value={currentSlide?.template === 'FADE' ? (currentSlide?.options?.fadeBlur ?? 0) : (currentSlide?.options?.backgroundBlur ?? (currentSlide?.template === 'CINEMATIC_BG' ? 0 : 12))} min={0} max={40} step={1} onChange={(v) => updateSlideProperty(currentSlide?.template === 'FADE' ? ['options', 'fadeBlur'] : ['options', 'backgroundBlur'], v)} />
                           </div>
                           {currentSlide?.template === 'FADE' && (
