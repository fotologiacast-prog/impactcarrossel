...
const readStringPreference = (source: Record<string, unknown>, keys: string[]): string | undefined => {
...
export default async function handler(_req: any, res: any) {
  try {
    ...
    const presets = (clients || []).map((client: any) => {
      ...
      const preferences = readJsonObject(client.preferences);
      const imageSettings = readJsonObject(client.image_settings);
